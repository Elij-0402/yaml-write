...
The store also holds view state: `selectedNovelId`, `workshopOpen` (show `FusionWorkshop`), `activeCreationId` (which creation-library record the workshop reads/writes — see Dexie v8), `workshopBusy` (set by `FusionWorkshop` while streaming/generating/colliding/tweaking; `page.tsx` blocks switching/creating a creation while it is true, to avoid cross-creation stale writes), and `manageMode` (show `NovelUploader`'s chapter list + re-split for the selected novel). `setSelectedNovelId` resets `workshopOpen`/`manageMode`/`activeCreationId`; `setActiveCreationId(id)` opens the workshop on that creation. There are **no tabs** and no cross-component chapter handoff — the right pane is chosen entirely from these flags in `app/page.tsx` (see Navigation & components).
...
- **`version(7)`**: Adds the `fusionSessions` table — originally a **singleton** (`id:'current'`, superseded by v8) persisting the fusion-workshop funnel (selected DNA ids, custom/adversarial prompts, step, directions, blocks, sceneCount, storyboard, per-scene prose + resume status) so a refresh or sidebar-click never蒸发 generated work.
- **`version(8)`**: Upgrades `fusionSessions` from the v7 singleton to a **multi-record "creation library"** (`'id, updatedAt, createdAt'`; adds `name` + `createdAt`). Each fusion run is its own record — `+ 新建创作` mints a fresh `crypto.randomUUID()` so a new run never overwrites prior work, and the sidebar's 创作 section (filtered to records that reached `creator` or have prose) loads/renames/deletes them. `FusionWorkshop.tsx` hydrates/writes by the store's `activeCreationId` (resets to a blank funnel for an unknown id); the idle write-back wraps get+put in one `rw` transaction so a sidebar rename can't be clobbered, treats the DB as the source of truth for `name`/`createdAt`, and skips empty sessions. The migration backfills `name`/`createdAt` on the existing `'current'` record — zero data loss.

Any change to the `Novel`/`Chapter`/`FusionSession` shape requires a new `this.version(n).stores(...).upgrade(...)` block — don't mutate existing version definitions. There is no server-side database. Components read reactively via `useLiveQuery` (`dexie-react-hooks`); mutations through `db.chapters.update(...)` propagate to all live queries automatically.

## Navigation & components

`app/page.tsx` is `'use client'` with **no tabs and no Next sub-routes** (SSR is not meaningfully exercised). The sidebar holds the novel library (each row shows a DNA-status badge via `dnaBadge`) **and a creation library** (the 创作 section — saved fusion runs; `+ 新建创作` opens a fresh one); the right pane is chosen from Zustand flags: `workshopOpen` → `<FusionWorkshop/>` (on the creation `activeCreationId`); else `selectedNovelId && !manageMode` → `<NovelDetail/>`; else `<NovelUploader/>`. `SettingsPanel` is a slide-over drawer opened via the `open-settings-panel` window event.
...
- **FusionWorkshop** — the 3-step fusion funnel: 引力室 (pick 1+ DNA-ready novels — **1 = 自我裂变, 2+ = 交叉融合**; optional 自定义大方向 / 反套路红队约束, the latter threaded into all four creation calls → `generate-fusion-directions`; exactly 2 books unlocks the gravity-orbit `fusionBias` slider) → three direction cards → creator (the 4 setting blocks + a command bar `tweak-fusion-blocks`, cyan-pulsing the changed blocks → a 1–8 分镜数 selector → `stream-storyboard` → per-scene streamed prose with 停止生成 / copy / save-as-TXT / 重写 / 继续接写, plus 导出全篇 to download the whole work — title + 4 setting blocks + all scene prose — as one `.md`). The funnel persists **per creation** to `db.fusionSessions` (keyed by the store's `activeCreationId`; see Dexie v8).
