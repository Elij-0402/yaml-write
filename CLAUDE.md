...
- Everything is a client component. The UI is a custom **活字 · Living Type** design system — warm paper (`--paper`) + ink (`--ink`), with one signal-teal accent (`--signal`) that lights only for "alive / happening now". CSS-variable tokens (`--ink`/`--muted`/`--faint`/`--hair`/`--surface`/`--shell`/`--well`/`--signal`/`--signal-soft`/`--danger`, `--font-serif`/`--font-sans`/`--font-mono`…) + ~40 semantic component classes (`.atelier`, `.eyebrow`, `.lede`, `.cta`, `.setcard`, `.slab`, `.recipe`, `.studio`, `.manuscript`…) defined in `app/globals.css`; the chrome (sidebar/top bar/stepper) uses Tailwind utilities bound to those tokens. **Visual iron rules:** signal teal marks only "alive / happening now" (extracting · streaming caret · active route · BYOK-connected), never static state or decoration; the primary action is a solid-ink button, one per screen; hierarchy comes from type scale / weight / whitespace / hairlines with almost no shadow (one ambient app-window shadow only); three typefaces stay in lane (serif reads · sans operates · mono measures); `danger` red is functional-only (real errors, e.g. flash-500 / storage failure). Legacy accent token names (`--add`/`--del`, `--ink-dim`, `--paper-text`…) survive only as compat aliases pointing at 活字 tokens. Match the surrounding file. `lucide-react` is installed but **no longer used** — icons are inline glyphs/emoji (墨, ✂︎, ✨, 🔧, 🎨, →). Respect `prefers-reduced-motion`.
