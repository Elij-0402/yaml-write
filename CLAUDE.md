...
`app/page.tsx` is `'use client'` with **no Next sub-routes** (SSR is not meaningfully exercised). The shell is a left **`AppRail`** (destination sidebar) + a `<main>` column with a **global 48px topbar** (breadcrumb: root segment clickable → back to its library; current entity plain text; plus a live `提取中 N` activity chip) above the section canvas. There is **no global stepper** — the current view is derived from Zustand flags + a local `homeSection` state: `workshopOpen` → `<FusionWorkshop/>` (Studio); else `selectedNovel` → `<NovelWorkspace/>`; else `homeSection==='creations'` → `<CreationsView/>`; else `<LibraryView/>`. `page.tsx` also hosts the global overlays: the **`CommandPalette`** (⌘/Ctrl+K, also via the sidebar search entry), the background-extraction "DNA 就绪" toast, the `persistError` hint, the `AppDialog`s (delete novel / delete creation / rename creation), the `SettingsPanel` slide-over (`open-settings-panel` event or ⌘/Ctrl+,), and the mobile nav drawer. `workshopBusy` blocks switching/creating a creation mid-stream (enforced in `CreationsView` rows + palette entries). Ghost-selection cleanup re-checks the DB before resetting `selectedNovelId` — the liveQuery array can lag a just-committed import (don't regress this to an array-only check). (Per-entity flow lives locally — `NovelWorkspace` segmented tabs + Studio's local step rail; the old global `WorkflowStepper` and `workflow.ts`'s `getNovelWorkflowSummary`/`store.engineNovelId`/`skinNovelId` were removed.)

- **AppRail** (`components/AppRail.tsx`) — Linear-style left sidebar (224px; mobile drawer + `--scrim`): brand → **search/palette entry** (⌘K hint → opens `CommandPalette`) → **作品库 / 创作库** (active = neutral raised, **not** accent) with mono counts → bottom **设置** + model-readiness dot (green/gray).
- **CommandPalette** (`components/CommandPalette.tsx`) — ⌘/Ctrl+K glass overlay: grouped commands (跳转 / 操作 / 作品 / 创作 / 主题), substring filter, ↑↓/Enter/Esc keyboard nav (skips disabled), `aria-activedescendant` listbox. Opens novels/creations, 新建创作 (gated on DNA-ready + `workshopBusy`), settings, and theme switching (`setThemePreference`).
- **LibraryView** (`components/LibraryView.tsx`) — the 作品库 page: an import dropzone (owns the upload → clean → split → blob-presplit → Dexie-write logic, moved out of `NovelUploader`) + a **row list** (`NovelCard` rows inside one hairline container, `divide-y`). Header has a 导入作品 button (same hidden file input). Empty state is **import-first** (hero dropzone + 3-step explainer). On import the novel lands in its 章节校验 workspace (`setManageMode(true)`).
- **NovelCard** (`components/NovelCard.tsx`) — a source-novel **list row** (h-12): status dot (提取中 = accent pulse / DNA 就绪 = success / else = neutral outline) + name + mono word count + status text + hover-reveal delete.
- **NovelWorkspace** (`components/NovelWorkspace.tsx`) — thin shell for one selected novel: title + word/chapter count + **`.seg` segmented tabs** `DNA` / `章节校验` (breadcrumb lives in the global topbar). `DNA` → `<NovelDetail/>`; `章节校验` → `<NovelUploader/>` (the `manageMode` review view).
- **CreationsView** (`components/CreationsView.tsx`) — the 创作库 page: a creation **row list** (filtered to `creator`/`manuscript`/has-prose; name + 骨架×题材 source line + step chip + mono time + hover actions copy/rename/delete) + `新建创作` (gated to ≥1 DNA-ready novel and `!workshopBusy`; mints a fresh `crypto.randomUUID()`). Row switching is blocked while `workshopBusy`.
...
- **FusionWorkshop** (the **Studio**) — the 4-step skin-swap funnel, wrapped in a local `StudioShell` (top step rail `1 配方 → 2 方向 → 3 设定 → 4 成稿` — current = solid-accent number square, done = check — + a single 下一步 hint; breadcrumb/back live in the global topbar), persisted **per creation** to `db.fusionSessions` (keyed by `activeCreationId`):
...
- **SettingsPanel** — slide-over (`.slide-in-right`): an **外观 section** (theme radio `.seg`: 跟随系统/亮色/暗色 via `app/theme.ts`'s `useTheme`) + BYOK guidance + `<ProviderCredentialsEditor variant="minimal" providerSelector="select" collapsibleAdvanced/>` + a "keys stay local" note. (Temperature defaults to 0.7; no slider.)
...
- Everything is a client component. The UI is a **Linear-style minimal design system** — **dark-first dual theme** (near-black cool canvas by default; fully calibrated light theme; preference in `localStorage('va-theme')` via `app/theme.ts` + a pre-paint inline script in `app/layout.tsx` writing `<html data-theme>`; "system" follows `prefers-color-scheme`), a **single low-saturation indigo accent** (`--accent` `#5E6AD2`), hairline borders, surface-luminance layering, 6/8/10px radii, ⌘K command palette, compact 32px controls, all-sans (serif only for the `.prose-reader` manuscript body). Full spec: **`design-system/MASTER.md`** (page overrides in `design-system/pages/`). CSS-variable tokens in `app/globals.css` (`--bg`/`--bg-subtle`/`--surface`/`--surface-2`/`--fg`/`--fg-2`/`--fg-3`/`--border`/`--border-2`/`--accent`(+`-hover`/`-fg`/`-ink`/`-subtle`)/`--danger`(+`-subtle`)/`--success`(+`-subtle`)/`--scrim`/`--surface-glass`/`--radius`(+`-sm`/`-lg`)/`--shadow-pop`/`--sans`/`--mono`/`--serif`) are defined per theme (`:root` = dark, `:root[data-theme='light']` = light) and mapped in `tailwind.config.js` to named colors (`canvas`/`panel`/`surface`/`raised`/`fg`/`fg-muted`/`fg-subtle`/`line`/`line-2`/`scrim`/`accent`(+`ink`)/`danger`/`success`); component classes live in `globals.css`'s `@layer components` (`.btn`(+variants), `.input`, `.card`, `.glass`, `.chip`, `.eyebrow`, `.kbd`, `.field-label`, `.seg`/`.seg-item`, `.prose-reader`, `.caret`, `.view-enter`, `.pop-enter`, `.slide-in-right`, `.skeleton`). **Visual iron rules:** the indigo accent marks only "主行动 / 聚焦 / 正在发生" (extracting pulse · streaming `.caret` · current step square), never decoration, and `--accent` is for **solid fills only** — colored *text/icons* use the per-theme calibrated `text-accent-ink` (likewise `danger`/`success` are theme-calibrated); one primary action per screen = solid-indigo `.btn-primary`; nav/tab current state = neutral raised, not accent; overlays use `--scrim` (never `bg-fg/45` — it turns white in dark) + `.glass` + the single `--shadow-pop`; hierarchy comes from type scale / weight / whitespace / hairlines / surface luminance; idle/ready use neutral gray dots; `danger` red is functional-only. Icons use **`lucide-react`** (one linear icon family) — emoji-as-icons forbidden. Match the surrounding file. Respect `prefers-reduced-motion`. Both themes must be checked when touching colors — never assume light values work in dark.
