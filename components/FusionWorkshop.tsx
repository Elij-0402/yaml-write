  ...
  return (
    <div className={`rounded-full border px-3 py-1 text-[10px] font-mono ${
      active 
        ...
        : 'border-hairline bg-transparent text-zinc-600'
    }`}>
      0{index}. {label}
    </div>
  );
  ...
  if (readyNovels.length < 2) {
    return (
      <div className="flex flex-1 items-center justify-center animate-fade-in bg-[#000000]">
        <div className="glass-card max-w-3xl rounded-xl p-8 text-center border-hairline bg-surface-1">
          ...
          <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
            ...
            <button
              ...
              className="rounded-xl border border-hairline bg-white/[0.015] px-5 py-2.5 text-xs font-semibold text-zinc-300 transition-linear hover:border-white/10 hover:bg-white/[0.03]"
            >
              前去模型配置检查
            </button>
          </div>
  ...
  if (step === 'material') {
    return (
      <div className="flex flex-1 flex-col gap-5 animate-fade-in">
        <div className="glass-card rounded-xl p-6 panel-grid border-hairline bg-surface-1">
        ...
        <div className="grid gap-4 xl:grid-cols-[1.5fr_0.9fr]">
          <div className="glass-card rounded-xl p-6 border-hairline bg-surface-1">
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="rounded-full border border-hairline bg-white/[0.01] px-3 py-0.5 text-[10px] font-mono text-zinc-450">
              ...
              {readyNovels.map((novel) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-xl border p-5 text-left transition-linear ${
                      selected
                  ...
                  >
                    <div className="flex items-center justify-between gap-4">
                      ...
                      {selected ? (
                        ...
                        <span className="rounded-full border border-hairline px-2.5 py-0.5 text-[9px] font-mono text-zinc-600">IDLE</span>
                      )}
                    </div>
          ...
          <div className="linear-card rounded-xl p-5 border-hairline bg-surface-2/40">
            ...
            <button
              ...
              className="mt-5 flex w-full items-center justify-between rounded-xl border border-hairline bg-white/[0.015] px-4 py-2.5 text-xs font-medium text-zinc-350 transition-linear hover:border-white/10 hover:bg-white/[0.03]"
            >
              <span>高级操控选项面板</span>
            ...
            {advancedOpen && (
              <div className="mt-4 space-y-3 rounded-xl border border-hairline bg-white/[0.01] p-4 text-xs">
                <textarea
                  value={customPrompt}
                  onChange={(event) => setCustomPrompt(event.target.value)}
                  rows={3}
                  placeholder="可选：引导碰撞走向，例如“把诡异废土的生存挣扎与高墙内部的权谋博弈压在同一条时间线上”..."
                  className="w-full rounded-xl border border-hairline bg-zinc-950 p-3 text-xs text-zinc-200 placeholder:text-zinc-700 focus:outline-none resize-none"
                />
                <textarea
            ...
            <div className="mt-5 rounded-xl border border-hairline bg-white/[0.01] p-4 text-xs leading-relaxed text-zinc-500">
            ...
            <div className="mt-5">
              {colliding ? (
                <div className="rounded-xl border border-hairline bg-zinc-950/40 p-4">
                ...
                <button
                  ...
                  className="w-full rounded-md bg-primary hover:bg-primary-hover active:bg-primary-focus px-5 py-3 text-xs font-semibold text-white transition-linear active-press disabled:cursor-not-allowed disabled:opacity-30 shadow-[0_0_8px_rgba(94,106,210,0.3)]"
                >
      ...
      <div className="flex flex-1 flex-col gap-5 animate-fade-in bg-[#000000]">
        <div className="flex items-center justify-between gap-4">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-xl border border-hairline bg-white/[0.015] px-4 py-2 text-xs font-medium text-zinc-400 transition-linear hover:border-white/10 hover:text-zinc-200"
          >
          ...
          <div className="mt-6 grid gap-5 xl:grid-cols-3">
            {directions.map((direction, index) => (
              <button
                ...
                className="group rounded-xl border border-hairline bg-white/[0.01] p-5 text-left transition-linear hover:border-white/15 hover:bg-white/[0.02]"
              >
  ...
  return (
    <div className="flex flex-1 flex-col gap-5 animate-fade-in bg-[#000000]">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-xl border border-hairline bg-white/[0.015] px-4 py-2.5 text-xs font-medium text-zinc-400 transition-linear hover:border-white/10 hover:text-zinc-200"
          >
            <ArrowLeft className="h-3.5 w-3.5" />
            重选融合走向
          </button>
      ...
      <div className="grid min-h-0 flex-1 gap-5 xl:grid-cols-[1.05fr_1fr]">
        <div className="flex min-h-0 flex-col gap-5">
          <div className="glass-card rounded-xl p-6 border-hairline bg-surface-1">
            <div className="flex items-center justify-between gap-3">
              ...
              <button
                onClick={() => setAdvancedOpen((value) => !value)}
                className="inline-flex items-center gap-1.5 rounded-xl border border-hairline bg-white/[0.015] px-3.5 py-2 text-xs font-medium text-zinc-350 transition-linear hover:border-white/10 hover:bg-white/[0.03]"
              >
                追加红队规约
            ...
            {advancedOpen && (
              <div className="mt-4 rounded-xl border border-hairline bg-white/[0.015] p-4">
                <textarea
                  value={adversarialRules}
                  onChange={(event) => setAdversarialRules(event.target.value)}
                  rows={3}
                  placeholder="追加不可涉足的禁区与约束，例如：“严防纯逻辑外挂、杜绝无端洗白与圣母走向”..."
                  className="w-full rounded-xl border border-hairline bg-zinc-950 p-3 text-xs text-zinc-200 placeholder:text-zinc-700 focus:outline-none resize-none font-mono"
                />
              </div>
            )}
              ...
              {BLOCKS.map(({ key, label, helper, icon: Icon }) => (
                <div
                  ...
                  className={`rounded-xl border bg-white/[0.01] p-5 transition-linear ${
                    pulse.has(key) ? 'pulse-zinc border-white/20' : 'border-hairline'
                  }`}
                >
                  <div className="flex items-center gap-3 border-b border-white/[0.02] pb-3">
                    <div className="rounded-xl border border-hairline bg-white/[0.015] p-2 text-zinc-300">
          ...
          <div className="shrink-0 glass-card rounded-xl px-5 py-4 border-hairline bg-surface-1 shadow-2xl flex flex-col">
            ...
            <div className="mt-3 flex flex-wrap gap-2 border-t border-white/[0.02] pt-3">
              {PRESETS.map((p) => (
                <button
                  ...
                  className="rounded-full border border-hairline bg-white/[0.01] px-2.5 py-1 text-[10px] text-zinc-550 transition-linear hover:border-white/15 hover:text-zinc-300"
                >
                  ✦ {p.label}
                </button>
              ))}
        ...
        <div className="flex min-h-0 flex-col gap-5">
          <div className="glass-card flex min-h-0 flex-1 flex-col rounded-xl p-6 border-hairline bg-surface-1">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-hairline pb-4">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-md bg-primary hover:bg-primary-hover active:bg-primary-focus px-4 py-2.5 text-xs font-semibold text-white transition-linear active-press disabled:opacity-50 shadow-[0_0_8px_rgba(94,106,210,0.3)]"
              >
                {generatingBoard ? (
            ...
            <div className="mt-5 flex-1 overflow-y-auto pr-0.5">
              {storyboard.length === 0 ? (
                <div className="flex h-full flex-col items-center justify-center rounded-xl border border-dashed border-hairline bg-white/[0.01] px-6 py-10 text-center">
                ...
                <div className="space-y-4">
                  {storyboard.map((scene) => (
                    <div key={scene.sceneNumber} className="rounded-xl border border-hairline bg-surface-2/40 p-5">
                        ...
                        <div className="mt-5 border-t border-hairline pt-4">
                          <div className="mb-3 flex items-center justify-end gap-2">
                            <button
                              ...
                              className="inline-flex items-center gap-1 rounded-full border border-hairline bg-white/[0.01] px-2.5 py-1 text-[10px] text-zinc-400 transition-linear hover:border-white/15 hover:bg-white/[0.02]"
                            >
